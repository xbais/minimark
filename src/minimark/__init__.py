...
def download_scm_file(repo_url, commit_hash, file_path, save_dir):
    """
...
def process_scm_repos_file(repos_file, file_to_download, save_dir):
    """
    ...
    with open(repos_file, "r") as file:
        for line in file:
            if line.strip():  # Skip empty lines
                ...
                download_scm_file(repo_url, commit_hash, file_to_download, save_dir)
        ...
        if self.type == '':
            ...
            try:
                ...
                if _lang == 'mermaid':
                    ...
                    _highlight_query = (Path(__file__).parent / "data" / f"{_lang}_highlights.scm").read_text()
...
def main():
    ...
    # Get the scm files for syntax highlighting
    # Path to the repos.txt file
    repos_file = str(Path(__file__).parent / "data" / "treesitter-grammar_repos.txt")
    # File to download from each repo
    file_to_download = "queries/highlights.scm"
    # Directory to save the downloaded files
    save_dir = str(Path(__file__).parent / "data")
    
    # Process the repos.txt file and download the required files
    process_scm_repos_file(repos_file, file_to_download, save_dir)
